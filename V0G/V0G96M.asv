...
%% 起始充电时间概率分布
us=70.4; %概率密度函数期望 17.6*4=70.4
ds=13.6; %概率密度函数标准差 3.4*4=13.6
%% 起始充电时间概率密度函数
fs1=@(x)1/(ds*(2*pi)^0.5).*exp(-(x-us).^2./(2*ds^2)); %（us-48)<x<=96
fs2=@(x)1/(ds*(2*pi)^0.5).*exp(-(x+96-us).^2./(2*ds^2)); %0<x<=(us-48)
xs=linspace(0,96,N);
%% 起始充电时间概率分布曲线
fs_ts=fs2(xs).*(xs<=us-48)+fs1(xs).*(xs>us-48);
s_ts=trapz(xs,fs_ts);  %计算整个区间概率密度的积分
%% 蒙特卡洛法计算起始充电时间概率分布
n=0;
while n<N
    t=rand(1)*96; %生成[0,96]均匀分布随机数
    if t<=us-48
        ft_ts=fs2(t)/s_ts;
    else
        ft_ts=fs1(t)/s_ts;
...
P_char_total = P_char_total(1:96)+P_char_total(97:192);
figure(5)
plot(P_char_total,'b-o','lineWidth',2);
title('电动汽车无序充电负荷曲线');
xlabel('充电时间(15min)');
ylabel('充电功率(kW)');