...
Result.V1G_Income_ALL=MC_params.B*sum(Result.V1G_ChargeLoad.*(BCS_params.EPrice-BCS_params.TOU_EPrice_Min));
...
hold on
title("电动汽车初始SOC分布")
plot(MC_params.nEVs_SOC,'*','Color',[0 0 1]);
xlabel('No.');
ylabel('SOC');
xlim([0 250]);
ylim([0 1]);
...
hold on
title("电动汽车预期充电时间分布")
plot(MC_params.nEVs_EChargeTime,'*','Color',[0 0 1]);
xlabel('No.');
ylabel('Time/15min');
xlim([0 250]);
ylim([0 50]);
