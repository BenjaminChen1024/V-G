...
V1G_params.C=zeros(BCS_params.nPiles,Basic_params.periods_per_day); %充电站状态矩阵
...
for t=1:Basic_params.periods_per_day
    ...
    if ~isempty(EV_index)
        ...
        yalmip('clear');
        % 定义矩阵
        V1G_params.X=sdpvar(BCS_params.nPiles,V1G_params.J_max);
        % 约束条件
        ...
        % 定义对象
        Algorithm.Objective=BCS_params.RCP*Basic_params.periods_Hour*sum(sum(V1G_params.X.*V1G_params.SC).*(BCS_params.EPrice-BCS_params.TOU_EPrice_Min(t)));
        % 为YALMIP求解器设置选项
        ...
        if Algorithm.sol.problem == 0
            Algorithm.solution = double(V1G_params.S);
        else
            Algorithm.solution = double(V1G_params.S);
